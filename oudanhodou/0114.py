...
class Hokousya_sita(Agent):
    ...
    def step(self):
        ...
        V_nomi = self.sokudo()
        dvdt = (V_nomi + hito_f + self.douro_f() ) / 16
            ...
            if kyori <= self.dmax and agents[i, 1] >= self.iti[1]:
                ...
                if tan_matome >= 0:
                    if tan_matome >= self.tan_siya:
                        fij = 0.45 * 9.8 * ((self.m /60 * 2) - kyori) * (nij)
                        fij = fij / self.m
    ...
    def syuui_f(self, num, agents):
        ...
        for i in range(num):
            ...
            tan_matome = (math.tan(self.siyakaku) + math.tan(self.a)) / (
                ...
                #if (-1) * math.tan(self.siyakaku) <= hito_tan and hito_tan < + math.tan(self.siyakaku):
                #if (-1) * tan_matome <= hito_tan and hito_tan <= tan_matome:
                    #fij = 5 * 9.8 * ((self.m/60 * 2) - kyori) *(nij)
                    #fij = fij / self.m
                    #mawari_hito = np.append(mawari_hito, fij)
            if kyori <= self.dmax and agents[i, 1] >= self.iti[1]:
            # if (-1) * math.tan(self.siyakaku) <= hito_tan and hito_tan < + math.tan(self.siyakaku):
...
class Zitensya_ue(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.iti = np.array([self.iti_x, self.iti_y])
        self.vi = np.array([0, -7.3])
        self.vdes = np.array([0, -7.3])
        self.m = 60
        self.tan_siya = math.tan(self.siyakaku)
...
class Oundahodou(Model):
    ...
    def __init__(self):
        ...
        self.syudan_zi_ue = np.zeros((self.num_zi_ue, 2))
        ...
        for i in range(self.num_zi_ue):
            d = Zitensya_ue(i, self)
            self.syudan_zi_ue[i, 0] = d.iti_x
            self.syudan_zi_ue[i, 1] = d.iti_y
            self.schedule.add(d)
    ...
    def dasu_syudan_hito(self):
        ...
        self.syudan_hito = np.append(self.syudan_hito, self.syudan_zi_ue, axis = 0)
...
anim.save('kotu_0114_anim.gif', writer='writer', fps=4)

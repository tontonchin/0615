...
class Hokousya_sita(Agent):
    ...
    def step(self):
        ...
        V_nomi = self.sokudo()
        dvdt = (V_nomi + hito_f + self.douro_f() ) /4
        ...
        for i in range(num):
            ...
            if nagasa <= self.dmax and kabe[i,1] >= self.iti[1] :
                if (-1) * math.tan(self.siyakaku) <= kabe_tan and kabe_tan < + math.tan(self.siyakaku):
                    fiw = 5000 * 9.8 * (self.m / 220 - nagasa)*(niw)
                    fiw = fiw / self.m
    ...
    def douro_f(self):
        fiw = 0
        if self.iti[0] <= 10:
            fiw = 5000 * 9.8 * (60/220 - self.iti[0])
        elif  self.iti[0] >= 40:
            fiw = 5000 * 9.8 * (60/220 - abs(self.iti[0]-10))

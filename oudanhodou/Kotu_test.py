...
class Hokousya(Agent):
    ...
    def step(self):
        ...
        if 0 <=self.iti_x  and self.iti_x <= 10:
            if 0 <= self.iti_y and self.iti_y <= 50:
        ...
        for i in range(num):
            ...
            if nagasa <= self.dmax and kabe[i,1] >= self.iti[1] :
                if (-1)*self.siyakaku <= kabe_tan and kabe_tan <+ self.siyakaku:
                    fiw = 0.0005 * 9.8 * (self.m / 360 - nagasa)*(niw)
                    fiw = fiw / self.m
    ...
    def douro_f(self):
        fiw = 0
        if self.iti[0] <= 10:
            fiw = 0.0005 * 9.8 * (60/360 - self.iti[0])
        elif  self.iti[0] >= 40:
            fiw = 0.0005 * 9.8 * (60/360 - (self.iti[0]-40))
...
anim = animation.ArtistAnimation(plt.figure(), ims, interval=500)
